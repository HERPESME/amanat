...
def _reference_caveat(facts: dict) -> str:
    refs = facts["refs"]
    caps = {r: n for r, n in refs.items() if r.startswith("CAP")}
    if len(caps) != 1:
        return ""
    (ref, n), = caps.items()
    return (f"Caveat from the stored records: every one of the {n} capture responses, across {facts['orders']} orders, carries the same "
            f"`action_reference` ({ref}), so the wording of the refusal of a second capture is probably a sandbox artefact. The outcome "
            "agrees with the vendor's documented rule that a transaction is captured or voided once.")
...
def _limits_line(doc: dict) -> str:
    ...
    if windows:
        bits.append(f"Visa {min(v for _, v in windows)}–{max(v for _, v in windows)} days by merchant category")
    return ", ".join(bits)
...
def _probe_facts(store_dir: Path | None) -> dict:
    """What the stored probe runs say about themselves: how many probes, how many holds, which references."""
        ...
        for _, rec in store.records(store.stream_path(name, store_dir)):
            ...
            for ex in rec["data"]["exchanges"]:
                ...
                if ex["label"] == "order_create" and isinstance(body, dict) and body.get("order_id"):
                    orders.add(body["order_id"])
                for item in (body if isinstance(body, list) else [body]):
                    ref = ((item or {}).get("authorization") or {}).get("action_reference") if isinstance(item, dict) else None
                    if ref:
                        refs[ref] += 1
    return {"probes": len(probes), "orders": len(orders), "refs": refs}
...
{st["rails"]} rails, {st["capabilities"]} capabilities, {st["limits"]} numeric limits. {st["observed"]} rows were **measured** against a
vendor's API; {st["reread"]} quotes were **re-read** from the source they cite on the date above; {st["unreadable"]} sources could not be read
...
- **A smaller capture than the hold** is supported on {len(partial["yes"])} rails and refused on {len(partial["no"])}
  ({_join(partial["no"])}); {len(partial["unk"])} have no evidenced answer.
- **Whether the rest is released without further action** is a real difference between rails, not a detail: supported on
  {_join(remainder["yes"])}; refused on {_join(remainder["no"])}; unverified on {_join(remainder["unk"])}.
- **A hold's life differs by rail**: {_limits_line(doc)}. After that the rail, not the agent, decides what becomes of the hold.
- **A retry that acts once** is documented for {len(idem["yes"])} rails ({_join(idem["yes"])}) and, separately, measured:
  {_measured_idempotency(doc)}.
...
{_regulator_note(unreadable)}Visa's guide is treated as secondary because it says the Visa Rules govern in any conflict, and the Rules have not been read.
