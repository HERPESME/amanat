...
*A dated snapshot of what payment rails say and do when an agent holds money, captures less than the hold, releases the rest, or retries. As of 2026-09-20.*
...
14 rails, 97 capabilities, 18 numeric limits. 12 rows were **measured** against a
vendor's API; 82 quotes were **re-read** from the source they cite on the date above; 15 sources could not be read
...
- 15 rows cite `www.npci.org.in (HTTP 403)`
